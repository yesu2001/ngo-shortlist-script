...
def run_pipeline(input_excel):
    ...
    total_rows = len(df)
    print(f"Total NGOs found in sheet: {total_rows}")
    print("Starting full processing of ALL NGOs...\n")
    ...
    for i in range(total_rows):
        ...
        if not name:
            print(f"[SKIP] Row {i} has empty NGO name, skipping.")
            continue

        print(f"\n=== {i+1}/{total_rows}: Processing NGO → {name} | {location}")
        ...
        # Polite sleep to avoid overload
        time.sleep(0.7)

    print("\n=== COMPLETED FULL PROCESSING ===")
    print(f"Total NGOs processed: {processed}")

    pd.DataFrame(shortlisted).to_excel("Shortlisted_NGOs_All.xlsx", index=False)
    pd.DataFrame(excluded).to_excel("Excluded_NGOs_All.xlsx", index=False)

    print("Generated:")
    print(" - Shortlisted_NGOs_All.xlsx")
    print(" - Excluded_NGOs_All.xlsx")
